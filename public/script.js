...
function openMonsterSelector(slot) {
    ...
    document.getElementById('selector-title').textContent = `Select Monster ${slot}`;
...
function selectMonsterFromGrid(monsterId) {
    const monster = monsters.find(m => m.id == monsterId);
    if (!monster) return;
    
    // Store selection
    if (currentSelectorSlot === 1) {
        window.selectedModalMonster1 = monsterId;
    } else {
        window.selectedModalMonster2 = monsterId;
    }
...
function validateModalSelection() {
    const monster1Id = window.selectedModalMonster1;
    const monster2Id = window.selectedModalMonster2;
...
function confirmMonsterSelection() {
    ...
    selectedMonsters[1] = monsters.find(m => m.id == monster1Id);
    selectedMonsters[2] = monsters.find(m => m.id == monster2Id);
